...
After any PR that touches message-mutation paths (`insertThreadMessages`, `deleteMessage`, `forkThread`, importers), also run:

```bash
pnpm integrity:check
```

That scans every thread for drift between the denormalized `messageCount` column and the real row count. Clean exit means the UI header and body will agree; drift means something bypassed the query layer. Add `--repair` (or use `pnpm integrity:repair`) to recompute.
...
- **Keep `messageCount` in sync.** Any new code path that inserts or deletes `operator_thread_messages` rows must also update `operator_threads.messageCount` in the same transaction. `forkThread` and `deleteMessage` are the reference patterns. Run `pnpm integrity:check` to verify.
- **No silent fallbacks on user-triggered actions.** If a button says "pull upstream" and we actually fall back to "forked from stored copy", return an explicit `outcome` field and surface it. See `lib/operator-studio/fork-upstream.ts` for the pattern.
