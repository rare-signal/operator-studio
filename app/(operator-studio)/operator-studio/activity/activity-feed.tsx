...
type ActivityTrigger = "user" | "system"
...
const KIND_META: Record<ActivityKind, KindMeta> = {
  ...
  "message.promoted": {
    label: "Promoted message",
    verb: "promoted a message in",
    icon: Flame,
  },
  "summary.created": {
    label: "Summary",
...
function ActivityRow({
  ...
  const isSystem = event.trigger === "system"

  // Special-case the assistant-reply chat message: an "AI replied to
  // me" event reads cleaner with the bot icon and a different verb
  // than "sent a message."
  const aiReply =
    event.kind === "chat.message.sent" && event.details.role === "assistant"
  const RowIcon = aiReply ? Bot : Icon
  const rowVerb = aiReply ? "received a reply in" : meta.verb
  const rowActor = aiReply
    ? typeof event.details.modelLabel === "string"
      ? event.details.modelLabel
      : "the assistant"
    : event.actor
    ...
    <li>
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div
            className={cn(
              "flex flex-wrap items-baseline gap-x-1.5 text-sm",
          ...
          >
            <strong
              className={cn(
                "font-semibold",
                isSystem ? "text-foreground/80" : "text-foreground"
              )}
            >
              {rowActor}
            </strong>
            ...
            <span
              className={cn(
                "font-medium",
                isSystem ? "text-foreground/85" : "text-foreground"
              )}
            >
              &ldquo;{title}&rdquo;
            </span>
          ...
          <div className="text-xs text-muted-foreground flex items-center gap-2">
            ...
            {isSystem && (
              <span
                ...
                title="Captured automatically by the studio (not a deliberate operator action)"
              >
                · auto
              </span>
            )}
...
function renderExtras(event: ActivityEvent): React.ReactNode {
  ...
  if (event.kind === "thread.appended") {
    const messageCount = event.details.messageCount
    const sourceApp = event.details.sourceApp
    return (
      <span className="inline-flex items-center gap-1.5">
        ...
        {typeof sourceApp === "string" && sourceApp.length > 0 && (
          <Badge
            variant="outline"
