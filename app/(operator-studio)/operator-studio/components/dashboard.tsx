...
export function Dashboard({ threads, stats }: DashboardProps) {
  ...
  React.useEffect(() => {
    ...
    // Sources with a server-side filesystem/db importer. Driven from
    // the importer registry via IMPORTER_SOURCE_IDS so adding OpenCode
    // (or any future source) doesn't require a separate edit here.
    const AUTOINGEST_SOURCES = IMPORTER_SOURCE_IDS
      ...
      try {
        for (const src of AUTOINGEST_SOURCES) {
          try {
            ...
            // Silent — auto-ingest is best-effort.
          }
        }
      } finally {
    ...
    // Skip the immediate kick on mount: the discover endpoint scans
    // every JSONL on disk synchronously per source, which can starve
    // Node's single thread on first navigation. The interval + the
    // visibilitychange handler still catch new activity within
    // intervalMs. Pulse-view follows the same pattern.
      ...
      {loading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Nothing promoted yet. Promote threads from the review flow, or promote standout messages directly.
            </p>
          </CardContent>
        </Card>
      ) : (
        ...
        <div className="space-y-4 flex-1 overflow-y-auto min-h-0">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={
                mode === "discover" &&
                !(IMPORTER_SOURCE_IDS as readonly string[]).includes(source)
                  ? IMPORTER_SOURCE_IDS[0]
                  : source
              }
              onValueChange={(v) => {
                setSource(v)
            ...
            >
              ...
              <SelectContent>
                {mode === "discover" ? (
                  // Discover has a real filesystem/db importer — only
                  // list sources where that importer exists. Derived
                  // from the registry so adding an importer auto-shows
                  // it here without a separate edit.
                  <>
                    {IMPORTER_SOURCE_IDS.map((id) => (
