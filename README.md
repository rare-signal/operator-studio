...
Operator Studio is organized around a few durable primitives:
...
- **Grounded continuation chat** *(experimental, opt-in)* — routes through any OpenAI-compatible `/v1/chat/completions` endpoint. Off by default; enable by setting `WORKBOOK_CLUSTER_ENDPOINTS`. The prompt is grounded in the thread's messages, summaries, and your promoted highlights. **Untested at v1 — treat as a preview surface; expect rough edges.**
...
## Grounded continuation chat *(experimental)*

> **Heads up:** the chat surface is shipped but not exercised in v1. The
> server route, fork-aware UI, persona selector, and SSE streaming all
> exist and *should* work, but we haven't run the integration through
> its paces against a live LLM endpoint. Expect rough edges, and please
> open issues with what you find.

When `WORKBOOK_CLUSTER_ENDPOINTS` is unset (the default), no chat input
or persona selector is rendered — threads read as frozen artifacts.
...
Configure via `WORKBOOK_CLUSTER_ENDPOINTS` (comma-separated URLs) and `WORKBOOK_CLUSTER_MODEL`. Streaming is on by default; pass `?stream=0` or use `Accept: application/json` for the non-streaming variant.
